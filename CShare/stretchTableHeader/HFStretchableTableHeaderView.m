...
@synthesize xstableView = _xstableView;
...
- (void)stretchHeaderForTableView:(UITableView*)tableView withView:(UIView*)view subViews:(UIView*)subview
{
    ...
    _xstableView.tableHeaderView = emptyTableHeaderView;

    [_xstableView addSubview:_view];
    [_xstableView addSubview:subview];
}

- (void)scrollViewDidScroll:(UIScrollView*)scrollView
{
    CGRect f     = _view.frame;
    f.size.width = _xstableView.frame.size.width;
    _view.frame  = f;
    ...
    if(scrollView.contentOffset.y < -64)
    {
        ...
        initialFrame.size.width  = _xstableView.frame.size.width + offsetY;
...
- (void)resizeView
{
    initialFrame.size.width = _xstableView.frame.size.width;
    _view.frame = initialFrame;
}
